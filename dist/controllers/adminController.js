...
const getStats = async (req, res, next) => {
    try {
        const [totalUsers, totalRestaurants, totalReviews, totalFavorites, topDistrictResult, mostReviewedResult,] = await Promise.all([
            User.countDocuments(),
...
const updateRestaurant = async (req, res, next) => {
    try {
        ...
        const result = await Restaurant.findByIdAndUpdate(id, { $set: update }, { new: true, runValidators: true }).lean();
        if (!result)
...
const updateRestaurantFood = async (req, res, next) => {
    try {
        ...
        for (let i = 0; i < foods.length; i++) {
            ...
            if (f && f._id && String(f._id) === targetKey) {
                idx = i;
                break;
            }
            if (f && f.id !== undefined && String(f.id) === targetKey) {
                idx = i;
                break;
            }
            // case-insensitive trimmed name match
            if (f && f.name && String(f.name).trim().toLowerCase() === lcKey) {
                idx = i;
                break;
            }
            // numeric index match
            if (!isNaN(Number(targetKey)) && Number(targetKey) === i) {
                idx = i;
                break;
            }
        }
        ...
        if (name !== undefined)
            target.name = name;
        if (price !== undefined)
            target.price = typeof price === 'number' ? price : Number(price) || 0;
        if (category !== undefined)
            target.category = category;
        if (image !== undefined)
            target.image = image;
        if (rating !== undefined)
            target.rating = normalizeFoodRating(rating);
        // ensure subdocs have _id (for older documents that lacked them)
        restaurant.foods.forEach((f) => { if (!f._id)
...
const deleteRestaurantFood = async (req, res, next) => {
    try {
        ...
        for (let i = 0; i < foods.length; i++) {
            const f = foods[i];
            let match = false;
            if (f && f._id && String(f._id) === key)
                match = true;
            if (!match && f && f.id !== undefined && String(f.id) === key)
                match = true;
            if (!match && f && f.name && String(f.name).trim().toLowerCase() === lcKey)
                match = true;
            if (!match && !isNaN(Number(key)) && Number(key) === i)
                match = true;
            if (match) {
                removed = true;
                continue;
